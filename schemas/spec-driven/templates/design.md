...
наименьшим компонентам-владельцам из закрытого словаря (класс — ровно один
компонент, одно требование могут нести несколько), слою, seam, который его
...
Repository, Bloc — другие Bloc; DTO доходит только до Repository, который
превращает его в Entity, и не попадает в domain и presentation; репозитории
...
[Условный список: оставьте пункты про mini, только если бэкенд change
построен на mini; иначе удалите их и опишите фактический подход к бэкенду
в этом же разделе. Опишите только существенные решения и применимые способы
реализации на mini:
