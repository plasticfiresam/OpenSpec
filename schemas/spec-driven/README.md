...
запуска `openspec validate <name> --strict`: незаменённая заготовка проверку
не проходит — Scenario с таким именем не называет ни одного теста. Снимает её
автор: `--strict` требует у требования хотя бы один Scenario и написанный
Purpose, а текст заготовки для него неотличим от написанного.
...
шаблоне построен. Шаблон Surf несёт такой файл сам; ниже — пример для проекта,
которому нужно написать его вручную.
...
context: |
  ...
  репозитории не импортируют другие репозитории, блоки — другие блоки;
...
артефакт (`proposal`, `specs`, `design`, `tasks`); лишние ключи `openspec`
предупреждением отбрасывает. Что показывает CLI:
